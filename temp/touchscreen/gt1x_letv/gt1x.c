...
static ssize_t gt1x_gpio_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int irq_status = -1;
    int reset_status = -1;
    ssize_t num_read_chars = 0;

	irq_status = gpio_get_value(gt1x_int_gpio);
	reset_status = gpio_get_value(gt1x_rst_gpio);
    num_read_chars = snprintf(buf, PAGE_SIZE, "irq_status=%d reset_status=%d  \n",irq_status,reset_status);
    return num_read_chars;
}
static  ssize_t gt1x_gpio_store(struct device *dev,
...
static DEVICE_ATTR(tp_info, S_IRUGO, gt1x_gpio_show, gt1x_gpio_store);
...
static int gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(!ret && ts_pinctrl ){
		...
		if( ret <0){
			pr_err("failed to select pin to active state \n");
		}
	...
	if (IS_ERR(thread)) {
       ...
       cdev.name = "touchpanel";
       cdev.groups = attr_groups;
       ret = letv_classdev_register(&client->dev,&cdev);
        if(ret){
               pr_err("Failure   creating classdev\n");
       }
