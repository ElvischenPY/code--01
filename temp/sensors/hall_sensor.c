...
/*******************************************************************************
*  Name:   hall_status_show
*  Brief:  show the status of hall :  0:far    1:near 
*  Input:
*  Output: 
*  Return: 
*******************************************************************************/
...
{
	struct hall_data *data = gl_hall_data;

	return snprintf(buf, PAGE_SIZE, "%d \n",data->hall_status);
}
static ssize_t hall_status_store(struct device *dev, struct device_attribute *attr,const char *buf,size_t len)
{
	return 0;
}
static DEVICE_ATTR(status,0664,hall_status_show,hall_status_store);
...
static int hall_driver_probe(struct platform_device *dev)
{
	...
	err = sysfs_create_group(&data->hall_dev->dev.kobj,&hall_attr_group);
	if(err<0){
		dev_err(&dev->dev,"sysfs_create_group: Error to create calibration attribute. \n");
		goto err_sysfs_create_group;
	}
	err = sysfs_create_link(data->hall_dev->dev.kobj.parent,&data->hall_dev->dev.kobj,"hall");
	if(err)
	{
		pr_err(" sysfs_create_link failed create [hall] link\n");
		goto err_sysfs_create_link;
	}
	pr_info("%s:[%d]  probe end \n",__func__,__LINE__);
	return 0;
err_sysfs_create_link:
sysfs_remove_link(data->hall_dev->dev.kobj.parent, "hall");
err_sysfs_create_group:	
	sysfs_remove_group(&data->hall_dev->dev.kobj,&hall_attr_group);
err_regulator_init:
