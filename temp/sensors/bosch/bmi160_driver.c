...
#define BMI160_VDD_MIN_UV  2600000
#define BMI160_VDD_MAX_UV  3300000
#define BMI160_VIO_MIN_UV  1800000
#define BMI160_VIO_MAX_UV  1800000
...
static int sensor_regulator_configure(struct bmi_client_data *data, bool on)
{
	int rc;
	...
	if (!on) {
		...
		if (regulator_count_voltages(data->vdd) > 0) {
			rc = regulator_set_voltage(data->vdd,
				BMI160_VDD_MIN_UV, BMI160_VDD_MAX_UV);
			if (rc) {
				dev_err(data->dev,
		...
		if (regulator_count_voltages(data->vio) > 0) {
			rc = regulator_set_voltage(data->vio,
				BMI160_VIO_MIN_UV, BMI160_VIO_MAX_UV);
			if (rc) {
				dev_err(data->dev,
...
static int sensor_regulator_power_on(struct bmi_client_data *data, bool on)
{
	int rc = 0;
...
int bmi_probe(struct bmi_client_data *client_data, struct device *dev)
{
	...
	sensor_regulator_configure(client_data, true);
	sensor_regulator_power_on(client_data, true);
