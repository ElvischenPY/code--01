...
#define AKM_INPUT_DEVICE_NAME	"akm09911"
	...
	int hard)
{
	...
	if (hard != 0) {
		pr_info("akm chip reset\n");
		gpio_direction_output(akm->gpio_rstn, 0);
		mdelay(5);
		gpio_direction_output(akm->gpio_rstn, 1);
		mdelay(5);
		pr_info("akm reset pin status = [%d]\n",gpio_get_value(akm->gpio_rstn));
			...
			struct akm0911_mdata_s16 *value)
{
	...
	err = AKECS_GetData_Poll(akm, buffer, AKM_SENSOR_DATA_SIZE);
#if 0
	/* Read status */
	buffer[0] = AKM_REG_STATUS;
				...
				char const *buf, size_t count)
{
	...
	data = data ? 1 : 0;
	dev_err(&client->dev, "%s enable = %ld\n",__func__,data);
	mutex_lock(&akm->mutex_enable);
	if (data != akm->enable) {
		if (data) {
...
//	dev_err(&client->dev, "%s op_mode = %ld\n",__func__, op_mode);
...
static ssize_t akm_place_store(
	struct device *dev, struct device_attribute *attr,
				char const *buf, size_t count)
{
				...
				struct akm_compass_data *akm)
{
	...
	pr_info("akm,gpio_rstn = [%d]", akm->gpio_rstn);
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
	gpio_request(s_akm->gpio_rstn, "akm,rst");
	...
	pr_info("akm09911_i2c_check_device in\n");
	err = akm09911_i2c_check_device(client);
	if (err < 0)
		goto exit4;
	pr_info("akm09911_i2c_check_device out\n");
