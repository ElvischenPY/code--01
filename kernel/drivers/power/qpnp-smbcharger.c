...
#define THERMAL_DISCHARGE_VOL_LEVEL   (3)
								...
								int lvl_sel)
{
	...
	if((chip->thermal_discharge_vol_levels > 0) && (chip->thermal_discharge_vol != NULL) &&
		...
		if((lvl_sel == THERMAL_DISCHARGE_VOL_LEVEL)
				&& get_battery_voltage() >= chip->thermal_discharge_vol[lvl_sel]) {

			pr_warn("We got to discharging due to over temperature!!!");
...
static void smbchg_letv_pd_set_vol_cur_work(struct work_struct *work)
{
	...
	if (chip->prev_black_call_mode == 0)
...
/*
  ...
  */
#define PD_CHARGER_INIT_WORK_DELAY 6000
...
#ifdef CONFIG_LETV_HW_DEV_DCT
	set_hw_dev_flag(DEV_PERIPHIAL_CHARGER_PMI);
#endif
	return 0;
